...
test('should mock forEach function', () => {
  ...
  expect(forEachCallbackMock.mock.calls[0][0]).toBe(4);
  expect(forEachCallbackMock.mock.calls[1][0]).toBe(3);

  // test the return value/s see line 5
  expect(forEachCallbackMock.mock.results[0].value).toBe(5);
  expect(forEachCallbackMock.mock.results[1].value).toBe(4);
